...
- Web console Permission Changes now keeps the process panel visible at 1024px desktop widths and switches the context and task strips to a single-column mobile layout without mixing approval sub-status into the journey header.
- Web 控制台权限变更页现在在 1024px 中窄桌面宽度仍保留处理流程栏，并在移动端将上下文条和任务条切成单列，同时不再把审批子状态混入旅程页头。
