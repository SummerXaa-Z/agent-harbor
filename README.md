...
| `AGENT_HARBOR_DEPLOYMENT_MODE` | Optional deployment mode. Set to `production` for deployment-style preflight checks that block development-only admin bypass, private-upstream flags, and weak bootstrap admin keys before startup, disable default local CORS origins, and warn when persistent storage is not configured. 可选部署模式；设置为 `production` 后，启动前会执行部署预检并阻断开发专用的管理绕过、私有上游开关和弱引导管理员密钥，关闭默认本地 CORS 来源，并在未配置持久化存储时输出预警。 |
