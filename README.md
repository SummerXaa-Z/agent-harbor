...
- **Approval and evidence / 审批与证据**: route approval queues to configured reviewers, expire and consume approval requests, record applied package evidence, and keep audit trails for every privileged permission change. 将审批队列路由给已配置审查员，对审批请求设置过期和一次性消费，记录权限包应用证据，并为高风险权限变更保留审计链。
