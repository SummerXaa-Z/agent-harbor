...
- PostgreSQL migration `015_permission_package_requested_capability.sql` adds the new provenance field with an empty-string default, so existing rows remain legacy bundle records. Version-1 applications remain available as history but do not satisfy current v2 readiness. Handoff tokens tied to those pre-v2 applications can still be listed and revoked, but they no longer authorize governed MCP tool calls. Re-preview, re-approve when required, apply the v2 package, and issue a fresh token. See the [v0.3.0 PRD](docs/product/0.3.0-access-handoff-prd.md) and [v0.3.0 release notes](docs/engineering/0.3.0-access-handoff-release-notes.md). / PostgreSQL 迁移会把新来源字段以空字符串默认值加入现有记录，因此旧记录仍保持整包语义；v1 应用仍可查询历史，但不能满足当前 v2 上线就绪。绑定这些旧应用的交付 Token 仍可列出和撤销，但不再能授权受治理的 MCP 工具调用。请重新预览、按需审批、应用 v2 权限包并签发新 Token。
