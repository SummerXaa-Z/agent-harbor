...
AgentHarbor changes should be easy to review in small slices, even when the branch is a larger integration checkpoint. Use this guide with `.github/pull_request_template.md` when preparing or reviewing pull requests.

## Preferred PR Shape

- One PR should represent one behavior slice: a domain contract, a management API, a data-plane behavior, a frontend workflow, or a documentation/scenario update.
- A stacked or integration PR is acceptable as a draft checkpoint, but the PR description must say so and list the review order.
...
## Review Order For Integration Branches
...
5. Docs and scenarios: verify README, changelog, and scenario scripts describe behavior that the code actually implements.
...
Run the relevant scenario script when a user-facing workflow changes. Scenario scripts should prove the public behavior and fail if expected evidence is missing.

## Security And Governance Checks

- Credential values, Agent Key plaintext, tokens, cookies, and authorization headers must not appear in management responses, audit metadata, traces, docs, or scenario output.
...
- Scenario/docs updates describe behavior from unrelated product areas.
