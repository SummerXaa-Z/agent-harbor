...
| vd-7 排版/空状态细节 | 空状态、行高、标题层级仍有继续优化空间 | 本轮只修 token 和焦点基础设施,未展开空状态重构 | 保留后续 |
...
- 没有全量重构所有空状态、表格密度和宽屏表单布局。当前先建立测试护栏,降低后续页面级优化风险。
...
4. **vd-7(空状态/行高/标题层级)**:确认未闭环,实测表格行高 81px、空状态仍是单行灰字。下轮 P1。
