...
import { capabilityDisplayName, readableIdentifierLabel, type Translator } from "../consolePresenters";
...
export function AskAccessView({
  ...
  const workspaceOptions = uniqueOptions(agents.map((agent) => agent.workspaceId), workspaceLabel);
  const callerOptions = agentOptions(agents.filter((agent) => agent.channelType === "local" || agent.status === "active"));
  const targetIds = new Set(capabilities.map((capability) => capability.targetId));
  const targetOptions = agentOptions(agents.filter((agent) => targetIds.has(agent.id)));
    ...
    <div className="ask-access">
      ...
      <section className="ask-query-panel" aria-label={t("ask.questionLabel")}>
        <div className="ask-query-sentence">
          ...
          />
          <span>{t("ask.questionCanAccess")}</span>
          <QueryField
      ...
      <section className="ask-answer" aria-label={t("ask.answerTitle")}>
        {!result ? (
          <EmptyRow
            actionLabel={exampleAvailable ? t("action.runExampleQuery") : undefined}
            detail={liveDataAvailable ? t("ask.emptyDetail") : t("ask.liveApiRequiredDetail")}
            onAction={exampleAvailable ? onRunExampleQuery : undefined}
      ...
      <section className="ask-history" aria-label={t("ask.historyTitle")}>
        <header>
...
function workspaceLabel(workspaceId: string) {
  return readableIdentifierLabel(workspaceId);
}
...
function historyLabel(
  ...
  const caller = agents.find((agent) => agent.id === request.callerInstanceId)?.name ?? request.callerInstanceId;
  const target = agents.find((agent) => agent.id === request.targetId)?.name ?? request.targetId;
