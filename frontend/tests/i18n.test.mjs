...
test("createTranslator returns Chinese labels for AI admin permission packages", () => {
  ...
  assert.equal(t("section.permissionApplicationHealth"), "应用健康");
  assert.equal(t("action.reviewApplicationImpact"), "复盘影响");
  assert.equal(t("action.refreshApplicationHealth"), "刷新健康");
  assert.equal(t("action.rehearseApplicationDrift"), "演练漂移");
  assert.equal(t("message.permissionApplicationImpactLoaded"), "应用影响已加载。");
  assert.equal(t("message.permissionApplicationHealthLoaded"), "应用健康已加载。");
  assert.equal(t("message.permissionApplicationHealthRequiresLiveApi"), "应用健康巡检需要实时 API。");
  ...
  assert.equal(t("metric.readyApplications"), "健康应用");
  ...
  assert.equal(t("empty.permissionApplicationHealth.detail"), "应用权限包后刷新健康状态。");
