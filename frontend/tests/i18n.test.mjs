...
test("English product copy uses acceptance and records wording", () => {
  ...
  assert.equal(
    ...
    "Management MCP tool catalog summary is incompatible with this console. Upgrade the AgentHarbor API before continuing."
  );
  assert.equal(
  ...
  assert.equal(t("systemCapability.managementMcpToolsMetadataV4"), "Management MCP tool confirmation-schema metadata");
...
test("createTranslator returns core journey Chinese labels", () => {
  ...
  assert.equal(t("resource.selectedRow"), "当前关注");
...
test("createTranslator returns Chinese labels for AI admin permission packages", () => {
  ...
  assert.equal(t("systemCapability.managementMcpToolsMetadataV4"), "Management MCP 工具确认参数元数据");
