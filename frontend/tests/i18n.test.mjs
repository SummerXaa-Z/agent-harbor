...
test("createTranslator returns Chinese labels for AI admin permission packages", () => {
  ...
  assert.equal(t("text.aiAdminProductionConsoleHelp"), "按申请、审批、落地和运行验证汇总上线状态。");
  ...
  assert.equal(t("section.permissionWizardApproval"), "提交审批");
  assert.equal(t("section.permissionWizardApply"), "应用权限");
  assert.equal(t("section.permissionWizardGoLive"), "上线就绪");
  ...
  assert.equal(t("section.permissionProductionReadiness"), "上线就绪状态");
  assert.equal(t("action.reviewApplicationImpact"), "查看影响");
  assert.equal(t("action.refreshApplicationHealth"), "刷新状态");
  assert.equal(t("action.checkProductionReadiness"), "检查上线就绪");
  ...
  assert.equal(t("message.permissionProductionReadinessLoaded"), "上线就绪状态已加载。");
  ...
  assert.equal(t("message.permissionProductionReadinessRequiresLiveApi"), "上线就绪检查需要实时 API。");
  ...
  assert.equal(t("empty.permissionProductionReadiness.detail"), "应用权限并采集运行、审计和落地证据后检查上线就绪。");
