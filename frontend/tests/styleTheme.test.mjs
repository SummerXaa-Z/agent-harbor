...
test("empty resource lifecycle uses inline guidance without duplicate empty cards", () => {
  ...
  assert.match(styles, /\.resource-lifecycle-setup-gap\.is-action\s*\{/);
  assert.match(styles, /\.resource-lifecycle-setup-gap\.is-action:focus-visible\s*\{/);
});
...
test("capability workspace compresses metrics and opens grant operations on demand", () => {
  ...
  assert.match(capabilityGovernanceView, /className="primary-button capability-review-launcher"/);
  assert.match(capabilityGovernanceView, /className="capability-grant-blocker"/);
  assert.match(styles, /\.capability-grant-blocker\s*\{/);
