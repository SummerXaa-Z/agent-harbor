...
RUN_ID="${RUN_ID:-sprint13-$(date +%Y%m%d%H%M%S)}"
...
if kind == "caller":
    ...
    body = {
        "tenantId": tenant_id,
        "workspaceId": workspace_id,
        "name": f"Sprint13 MCP Target {run_id}",
        "channelType": "mcp",
        "status": "active",
        "channelConfig": {
            "endpoint": endpoint,
...
echo "AgentHarbor Sprint 13 data permission enforcement demo"
...
if [[ -z "$MCP_ENDPOINT" ]]; then
  echo "MCP_ENDPOINT is not set; skipping Sprint 13 demo because capability discovery needs a safe test MCP HTTP endpoint."
  echo "Set MCP_ENDPOINT to an endpoint exposing $ALLOWED_TOOL to run the full demo."
  exit 0
fi
...
request POST "/api/v1/agent-keys" "{\"agentId\":\"$CALLER_ID\",\"name\":\"sprint13 key\"}"
...
echo "data permission enforcement demo complete"
