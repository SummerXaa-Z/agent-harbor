...
ADMIN_KEY="${ADMIN_KEY:-}"
RUN_ID="${RUN_ID:-sprint15-$(date +%Y%m%d%H%M%S)}"
...
WORKSPACE_ID="${WORKSPACE_ID:-ws-sprint15}"
...
echo "AgentHarbor Sprint 15 tenant access profile demo"
...
request POST "/api/v1/tenants" "$(json_body tenant "$ROOT_TENANT_ID" "" "Sprint15 Root")"
expect_status 201 "create root tenant"
request POST "/api/v1/tenants" "$(json_body tenant "$CHILD_TENANT_ID" "$ROOT_TENANT_ID" "Sprint15 Child")"
expect_status 201 "create child tenant"
request POST "/api/v1/tenants" "$(json_body tenant "$GRANDCHILD_TENANT_ID" "$CHILD_TENANT_ID" "Sprint15 Grandchild")"
expect_status 201 "create grandchild tenant"

request POST "/api/v1/agents" "$(json_body agent "$CHILD_TENANT_ID" "$WORKSPACE_ID" "Sprint15 Child Caller" "local")"
expect_status 201 "create child caller"
CALLER_ID="$(json_get data.id)"

request POST "/api/v1/agent-keys" "{\"agentId\":\"$CALLER_ID\",\"name\":\"sprint15 key\"}"
expect_status 201 "create key"
AGENT_KEY="$(json_get data.key)"

if [[ -z "$MCP_ENDPOINT" ]]; then
  request POST "/api/v1/agents" "$(json_body agent "$ROOT_TENANT_ID" "$WORKSPACE_ID" "Sprint15 Local Target" "local")"
...
request POST "/api/v1/agents" "$(json_body agent "$ROOT_TENANT_ID" "$WORKSPACE_ID" "Sprint15 Root MCP" "mcp" "$MCP_ENDPOINT")"
...
echo "tenant access profile demo complete"
