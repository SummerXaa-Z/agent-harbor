...
RUN_ID="${RUN_ID:-sprint8-$(date +%Y%m%d%H%M%S)}"
OLD_SECRET="Bearer sprint8-old-$RUN_ID"
NEW_SECRET="Bearer sprint8-new-$RUN_ID"
...
if kind == "agent":
    ...
    body = {
        "name": f"Sprint8 Audited MCP {run_id}",
    ...
    run_id = sys.argv[2]
    body = {
        "name": f"Sprint8 Audited MCP Updated {run_id}",
        "description": "patched by sprint8 management audit demo",
        "status": "draft",
    }
elif kind == "rotate":
    secret = sys.argv[2]
...
echo "AgentHarbor Sprint 8 management audit demo"
...
echo "management audit demo complete"
