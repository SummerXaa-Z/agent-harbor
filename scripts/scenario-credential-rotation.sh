...
RUN_ID="${RUN_ID:-credential-rotation-$(date +%Y%m%d%H%M%S)}"
OLD_SECRET="Bearer credential-rotation-old-$RUN_ID"
NEW_SECRET="Bearer credential-rotation-new-$RUN_ID"
...
if kind == "agent":
    ...
    body = {
        "name": f"Credential Rotation Credentialed MCP {run_id}",
    ...
    run_id = sys.argv[2]
    body = {
        "name": f"Credential Rotation Patched MCP {run_id}",
        "description": "patched by credential-rotation scenario",
...
data = doc["data"]
if data["name"] != f"Credential Rotation Patched MCP {os.environ['RUN_ID']}":
    raise SystemExit(f"unexpected patched name: {data}")
if data["description"] != "patched by credential-rotation scenario" or data["ownerId"] != "platform-team" or data["status"] != "draft":
...
echo "AgentHarbor credential rotation scenario"
...
echo "credential rotation scenario complete"
