...
RUN_ID="${RUN_ID:-mcp-capability-$(date +%Y%m%d%H%M%S)}"
...
if kind == "caller":
    ...
    body = {
        ...
        "name": f"MCP Capability Caller {run_id}",
        "channelType": "local",
        "status": "active",
    }
elif kind == "target":
    ...
    body = {
        "tenantId": tenant_id,
        "workspaceId": workspace_id,
        "name": f"MCP Capability MCP Target {run_id}",
        "channelType": "mcp",
        "status": "active",
...
echo "AgentHarbor MCP capability governance scenario"
...
if [[ -z "$MCP_ENDPOINT" ]]; then
  echo "MCP_ENDPOINT is not set; skipping MCP capability scenario because AgentHarbor rejects loopback endpoints by design."
  echo "Set MCP_ENDPOINT to a safe test MCP HTTP endpoint exposing $ALLOWED_TOOL and $DENIED_TOOL to run the full scenario."
  exit 0
fi
...
request POST "/api/v1/agent-keys" "{\"agentId\":\"$CALLER_ID\",\"name\":\"mcp-capability key\"}"
...
echo "mcp capability governance scenario complete"
