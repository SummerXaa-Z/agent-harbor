...
RUN_ID="${RUN_ID:-mcp-policy-$(date +%Y%m%d%H%M%S)}"
...
if kind == "agent":
    ...
    body = {
        "agentId": agent_id,
        "name": f"mcp-policy-key-{run_id}",
        "expiresInSeconds": 900,
    }
elif kind == "grant":
    caller_id, target_id, route_key = sys.argv[2], sys.argv[3], sys.argv[4]
    body = {
    ...
    body = {
        ...
        "id": method,
        "method": method,
        "params": {"name": "scenario.method", "arguments": {"message": "hello"}},
    }
else:
...
traces = doc.get("data", [])
pairs = [(trace.get("routeKey"), trace.get("decision")) for trace in traces]
expected = [("tools/list", "allowed"), ("tools/call", "denied")]
for item in expected:
    if item not in pairs:
        raise SystemExit(f"missing trace {item}; got {pairs}")
print("method traces: " + ", ".join(f"{key}={decision}" for key, decision in pairs))
PY
}
...
echo "AgentHarbor MCP method policy scenario"
...
request POST "/api/v1/access-grants" "$(json_body grant "$CALLER_ID" "$TARGET_ID" "tools/list")"
expect_status 201 "create tools/list grant"
echo "created tools/list grant: $(json_get data.id)"

request POST "/api/v1/mcp/agents/$TARGET_ID/rpc" "$(json_body mcp-call tools/list)" "$AGENT_KEY" "$RUN_ID"
expect_status 200 "tools/list call"
echo "tools/list allowed"

request POST "/api/v1/mcp/agents/$TARGET_ID/rpc" "$(json_body mcp-call tools/call)" "$AGENT_KEY" "$RUN_ID"
expect_status 403 "tools/call call without grant"
echo "tools/call denied without matching method grant"

request GET "/api/v1/audit/traces?runId=$RUN_ID"
expect_status 200 "list run traces"
assert_method_traces

echo "method-policy scenario complete"
