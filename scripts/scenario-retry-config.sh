...
RUN_ID="${RUN_ID:-retry-config-$(date +%Y%m%d%H%M%S)}"
...
body = {
    "name": f"Retry Config Retry {kind} {run_id}",
...
echo "AgentHarbor retry configuration scenario"
...
echo "retry config scenario complete"
