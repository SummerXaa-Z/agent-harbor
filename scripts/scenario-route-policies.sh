...
RUN_ID="${RUN_ID:-route-policy-$(date +%Y%m%d%H%M%S)}"
...
if kind == "agent":
    ...
    body = {
        "name": f"Route Policy {role} {run_id}",
    ...
    body = {
        "agentId": agent_id,
        "name": f"route-policy-key-{run_id}",
...
echo "AgentHarbor route policy scenario"
...
request POST "/api/v1/route-policies" "$(json_body policy "Route Policy allow calls $RUN_ID" "$CALLER_ID" "$TARGET_ID" allow 100)"
...
request POST "/api/v1/route-policies" "$(json_body policy "Route Policy deny calls $RUN_ID" "$CALLER_ID" "$TARGET_ID" deny 200)"
...
echo "route policy scenario complete"
