...
RUN_ID="${RUN_ID:-route-retry-$(date +%Y%m%d%H%M%S)}"
...
if kind == "agent":
    ...
    body = {
        "name": f"Route Retry {role} {run_id}",
    ...
    body = {
        "name": "Route Retry retry override",
...
echo "AgentHarbor route policy retry scenario"
...
echo "route policy retry scenario complete"
