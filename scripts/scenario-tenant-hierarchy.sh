...
ADMIN_KEY="${ADMIN_KEY:-}"
RUN_ID="${RUN_ID:-tenant-hierarchy-$(date +%Y%m%d%H%M%S)}"
...
WORKSPACE_ID="${WORKSPACE_ID:-ws-tenant-hierarchy}"
...
echo "AgentHarbor tenant hierarchy scenario"
...
request POST "/api/v1/tenants" "$(json_body tenant "$ROOT_TENANT_ID" "" "Tenant Hierarchy Root")"
expect_status 201 "create root tenant"
request POST "/api/v1/tenants" "$(json_body tenant "$CHILD_TENANT_ID" "$ROOT_TENANT_ID" "Tenant Hierarchy Child")"
expect_status 201 "create child tenant"
request POST "/api/v1/tenants" "$(json_body tenant "$GRANDCHILD_TENANT_ID" "$CHILD_TENANT_ID" "Tenant Hierarchy Grandchild")"
expect_status 201 "create grandchild tenant"
request POST "/api/v1/tenants" "$(json_body tenant "$UNRELATED_TENANT_ID" "" "Tenant Hierarchy Unrelated")"
...
request POST "/api/v1/agents" "$(json_body agent "$ROOT_TENANT_ID" "$WORKSPACE_ID" "Tenant Hierarchy Root Agent" "local")"
expect_status 201 "create root agent"
request POST "/api/v1/agents" "$(json_body agent "$CHILD_TENANT_ID" "$WORKSPACE_ID" "Tenant Hierarchy Child Agent" "local")"
expect_status 201 "create child agent"
request POST "/api/v1/agents" "$(json_body agent "$GRANDCHILD_TENANT_ID" "$WORKSPACE_ID" "Tenant Hierarchy Grandchild Agent" "local")"
...
if [[ -z "$MCP_ENDPOINT" ]]; then
  echo "MCP_ENDPOINT is not set; tenant hierarchy scenario complete. Set MCP_ENDPOINT to verify parent-to-child capability entitlement."
  exit 0
fi

request POST "/api/v1/agents" "$(json_body agent "$ROOT_TENANT_ID" "$WORKSPACE_ID" "Tenant Hierarchy Root MCP" "mcp" "$MCP_ENDPOINT")"
...
echo "tenant hierarchy scenario complete"
